...
def rouge_multi(hypotheses, references):
  """Calculates average rouge scores for a list of hypotheses and
  multiple references"""
  ...
  # Calculate ROUGE-1 F1, precision, recall scores
  rouge_1 = [
      max([rouge_n([hyp], [ref], 1) for ref in refs], key=lambda r: r[0]) for hyp, refs in zip(hypotheses, references)
  ]
  rouge_1_f, rouge_1_p, rouge_1_r = map(np.mean, zip(*rouge_1))

  # Calculate ROUGE-2 F1, precision, recall scores
  rouge_2 = [
      max([rouge_n([hyp], [ref], 2) for ref in refs], key=lambda r: r[0]) for hyp, refs in zip(hypotheses, references)
  ]
  rouge_2_f, rouge_2_p, rouge_2_r = map(np.mean, zip(*rouge_2))

  # Calculate ROUGE-L F1, precision, recall scores
  rouge_l = [
      max([rouge_l_sentence_level([hyp], [ref]) for ref in refs], key=lambda r: r[0])
